...
-- the workouts are written, then checked, and if one lists an exercise that isn't a built-in one yet, the whole
-- block is undone.
do $$
declare
  missing text[];
...
-- An earlier version of this file used a temporary table; if a run of it left one behind, it goes.
